...
output_dir   <- file.path(base_path, "Data/Final/Taxonomy")
...
classify_run <- function(run_name, seqtab_path, output_dir, unite_db) {
  ...
  taxonomy <- assignTaxonomy(
    seqs,
    unite_db,
    multithread = TRUE,   # auf Linux-Server möglich!
    tryRC       = TRUE,
    minBoot     = 80,
    verbose     = TRUE
  )
...
# ----------------------------------------------------------------------------
# Alle vier Läufe klassifizieren
# ----------------------------------------------------------------------------
runs <- list(
  EC24_245 = file.path(base_path, "Data/Final/DADA2_Output/EC24_245/seqtab_nochim.rds"),
  `3XX`    = file.path(base_path, "Data/Final/DADA2_Output/3XX/seqtab_nochim.rds"),
  Popa     = file.path(base_path, "Data/Final/DADA2_Output/Popa/seqtab_nochim.rds"),
  Buse     = file.path(base_path, "Data/Final/DADA2_Output/Buse/seqtab_nochim.rds")
)
...
# ----------------------------------------------------------------------------
# Gemeinsame Taxonomie für seqtab_all
# ----------------------------------------------------------------------------
cat("\n========================================\n")
cat("Klassifiziere gemeinsame ASV-Tabelle\n")
cat("========================================\n")

seqtab_all <- readRDS(file.path(base_path, "Data/Final/DADA2_Output/seqtab_all.rds"))
